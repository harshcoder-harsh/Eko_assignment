...
- **Org isolation:** data (tickets, audit runs, etc.) is stored and queried scoped to the caller's `org_id`. A cross-org read returns `404` — one org cannot see or mutate another's data.
...
2. **Retrieve SOP context** — scoped to synced documents named with `sop`/`faq`/`policy`/`runbook`, reusing the existing FAISS index.
